...
type LoginPolicySpec struct {
	...
	//   be unique across all targetRef entries in the LoginPolicy.
	...
	// * If unspecified, This LoginPolicy will have the lowest possible precedence.
	//
	// +optional
	Priority int `json:"priority"`

	// Permission indicates the required permission for a Minecraft player in order for this
	// LoginPolicy to be applied to them.
	//
	...
	// LoginPolicy applies to all players unless restricted by other
	// fields.
	//
	// +optional
...
type LoginPolicy struct {
	...
	// spec defines the desired state of LoginPolicy
	// +required
	Spec LoginPolicySpec `json:"spec"`

	// status defines the observed state of LoginPolicy
	// +optional
	Status gatewayv1alpha2.PolicyStatus `json:"status,omitempty,omitzero"`
}
