...
The built-in integration uses [Velocity](https://papermc.io/software/velocity), but you can implement the [network integration API](/guides/network-integration) to use any proxy. The network proxy is the component that understands join routes, fallback routes, priorities, and distribution strategies.
...
7. The network proxy looks up the player's destination using its xDS snapshot. `MinecraftJoinRoute` rules are evaluated in priority order; `MinecraftFallbackRoute` rules apply when the primary server is unavailable.
8. The network proxy forwards the player to the selected game server.
