...
`NetworkInfrastructure` configures how a gateway discovers backend game servers, how its Velocity network proxy Deployments are templated, and (at the GatewayClass level) how the edge proxy DaemonSet is configured.
...
An optional `DeploymentSpec` fragment applied via strategic merge patch to the default Velocity proxy Deployment created for each listener.

Use this to mount secrets and config maps, set resource requests, or configure tolerations.
...
| Field | Type | Description |
|---|---|---|
| `daemonSet` | `DaemonSetSpec` | Applied via strategic merge patch to the default edge DaemonSet. The selector labels and bootstrap volume/mount are always enforced. |
