...
A `MinecraftFallbackRoute` defines where players are sent when they cannot reach their primary destination — for example, when a game server is full or offline. Fallback routes are evaluated by the Velocity network proxy and do **not** affect edge-layer hostname routing.
