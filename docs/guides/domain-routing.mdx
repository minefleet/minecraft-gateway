...
| Field | Purpose |
|---|---|
| `spec.hostnames` | **Availability** — declares which hostnames this route handles at the edge. The edge proxy uses this to know which incoming connections this route can accept. |
| `filterRules[].rules[].domain` | **Routing** — defines the domain patterns that map connections to this route's backends, including any conditions (`permission`, rule set type). |
...
Routes within the same `Gateway` can share domains — the controller evaluates them in priority order.
