...
The edge filter automatically strips Forge and BungeeCord/Velocity `\0`-separated suffixes from the `server_address` field before matching. For example, `play.example.com\0FML\0` is treated as `play.example.com`.