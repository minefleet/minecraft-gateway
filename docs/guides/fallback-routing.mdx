...
The controller evaluates `MinecraftFallbackRoute` rules in `priority` order when a player cannot reach their primary `MinecraftJoinRoute` backend.
...
The `fallbackFor` field selects which services this fallback applies to using a label selector, without hardcoding service names. The controller resolves the selector against EndpointSlice labels when it builds routing configuration.
