...
Player counts are read from **pod annotations** set on game server pods. The controller watches for annotation changes and rebuilds routing configuration immediately when they change.
...
<Note>
This is the capacity of one **game server pod**, used to decide where to route a player. It is unrelated to the number a server list ping advertises, which comes from the proxies' own configuration — see [`playerCount`](/reference/network-infrastructure#playercount).
</Note>
